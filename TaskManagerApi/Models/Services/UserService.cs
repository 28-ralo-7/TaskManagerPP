...
{
    public class UserService: AbsractionService, ICommonService<UserModel>
    {
        ...
        public Tuple<string,string> GetUserLoginPassFromBasicAuth(HttpRequest request)
        {
            ...
            {
                string encodedUserLoginPass = authHeader.Replace("Basic ", "");
                var encoding = Encoding.GetEncoding("iso-8859-1");
                string[] namePassArray = encoding.GetString(Convert.FromBase64String(encodedUserLoginPass)).Split(':');
        ...
        {
            var user = _db.Users.FirstOrDefault(n => n.Email == login && n.Password == password);
            return user;
        }

        public User GetUser(string login)
        {
            var user = _db.Users.FirstOrDefault(n => n.Email == login);
            return user;
        }
